...
def gt_controller(p, p_dot, p_d, p_d_dot, p_d_2dot, p_d_3dot, p_d_4dot, \
    ...
    A_transposed = A.reshape(1,3).squeeze()
    z_d_B = - A/LA.norm(A)
    B = np.cross(z_d_B.reshape(1,3), x_c.reshape(1,3)).reshape(3,1)                 # temporary variable
    ...
    if calculate_omega_d == False:
        ...
        A_dot = A_dot.reshape(1,3).squeeze()
        u_dot = -A_dot @ (R@e3) + A @ (R_dot@e3)
        e_p_3dot = -(1/m)*(u_dot * R@e3 + u * R_dot@e3 + m*p_d_3dot)
        A_2dot = -Kp*e_p_2dot - Kv*e_p_3dot + m*p_d_4dot

        normA = LA.norm(A)
        normA_dot = (A_transposed@A_dot)/normA
        normA_2dot = (1/normA)*(A_transposed@A_dot + A_transposed@A_2dot) - (1/normA**2)*A_transposed@A_dot*normA_dot
        ...
        B_dot = np.cross(z_d_B_dot.reshape(1,3) , x_c.reshape(1,3)) + np.cross(z_d_B.reshape(1,3), x_c_dot.reshape(1,3))
        B_dot = B_dot.reshape(3,1)
        B_2dot = np.cross(z_d_B_2dot.reshape(1,3), x_c.reshape(1,3)) + 2*np.cross(z_d_B_dot.reshape(1,3), x_c_dot.reshape(1,3)) + np.cross(z_d_B.reshape(1,3),x_c_2dot.reshape(1,3))
        B_2dot = B_2dot.reshape(1,3)
